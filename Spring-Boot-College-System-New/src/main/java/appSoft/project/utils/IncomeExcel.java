...
public class IncomeExcel extends AbstractXlsxView {
	...
	protected void buildExcelDocument(
		...
		response.addHeader("Content-Disposition", "attachment;filename=Income List.xlsx");
		
		//2. read data given by Controller
		@SuppressWarnings("unchecked")
		List<FeesPayment> list = (List<FeesPayment>) model.get("feesPaymentList");
		
		//3. create one sheet
		Sheet sheet = workbook.createSheet("INCOME LIST");
	...
	private void setBody(Sheet sheet, List<FeesPayment> list) {
		int rowNum = 1;
